...
void GameFrameworkClassDef::reloadData()
{
    ...
    for(iFiles = FrameworkData::gameScriptData.constBegin(); iFiles != FrameworkData::gameScriptData.constEnd(); iFiles++)
    {
        ...
        itemFile->setData(0,TIKIND,"file");
        ...
        for(iClasses = oFile.constBegin(); iClasses != oFile.constEnd(); iClasses++)
        {
            ...
            itemClass->setData(0,TIKIND,"class");
            ...
            for(iObjects = oClass.begin();iObjects != oClass.end();iObjects++)
            {
                ...
                itemMember->setData(0,TIKIND,"member"); //class member
...
void GameFrameworkClassDef::on_treeWidget_currentItemChanged(QTreeWidgetItem *item, QTreeWidgetItem *previous)
{
    ...
    QString type = item->data(0,TIKIND).toString();
    if(type=="member")
    {
...
void GameFrameworkClassDef::on_addvarRB_clicked()
{
    ...
    QString type = it->data(0,TIKIND).toString(); //file, class, member
...
void GameFrameworkClassDef::on_addFunction_clicked()
{
    ...
    n->setData(0,TIKIND,QString("member"));
    vObject o;
    o["kind"]="func";
    o["type"]="void";
    o["desc"]="";
...
void GameFrameworkClassDef::on_treeWidget_customContextMenuRequested(const QPoint &pos)
{
    ...
    else if(res==&del)
    {
        ...
        QString kind = item->data(0,TIKIND).toString(); //file,class,member
...
void GameFrameworkClassDef::on_TaskCurrentClass_clicked()
{
    ...
    saveAndReload();
    whichTask();
    Class = FrameworkData::getFrameworkClass(className);
...
void GameFrameworkClassDef::on_TaskAllClasses_clicked()
{
    ...
    QList<vObject> L;
    vObjectIterator FI;
    for(FI=FrameworkData::gameScriptData.begin();FI!=FrameworkData::gameScriptData.end();++FI)
        if(FI.key()==file)
        {
...
void GameFrameworkClassDef::on_TaskAllFiles_clicked()
{
    ...
    vObject Class;
    CALLTASKFUNC(Class,0);
    QList<vObject> L = FrameworkData::getAllFrameworkClasses();
